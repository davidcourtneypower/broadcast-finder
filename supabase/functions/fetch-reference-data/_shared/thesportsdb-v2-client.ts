...
export class TheSportsDBv2Client {
  ...
  /**
   * Fetch all countries
   * Uses V1 all_countries.php
   */
  async fetchAllCountries(): Promise<TheSportsDBCountry[]> {
    const url = `${this.baseUrlV1}/all_countries.php`;
    console.log(`[V1] Fetching all countries...`);

    try {
      const response = await this.fetchWithTimeout(url, false);
      if (!response.ok) {
        console.warn(`[V1] All countries API returned ${response.status}`);
        return [];
      }
      const data = await response.json();
      // TheSportsDB may use "countries" or "countrys" as the response key
      const countries: TheSportsDBCountry[] = data.countries || data.countrys || [];
      console.log(`[V1] Received ${countries.length} countries`);
      return countries;
    } catch (error) {
      console.warn(`[V1] Failed to fetch countries: ${(error as Error).message}`);
      return [];
    }
  }
  ...
  /**
   * Fetch all leagues
   * Uses V1 all_leagues.php
   */
  async fetchAllLeagues(): Promise<TheSportsDBLeague[]> {
    const url = `${this.baseUrlV1}/all_leagues.php`;
    console.log(`[V1] Fetching all leagues...`);

    try {
      const response = await this.fetchWithTimeout(url, false);
      if (!response.ok) {
        console.warn(`[V1] All leagues API returned ${response.status}`);
        return [];
      }
      const data = await response.json();
      const leagues: TheSportsDBLeague[] = data.leagues || [];
      console.log(`[V1] Received ${leagues.length} leagues`);
      return leagues;
    } catch (error) {
      console.warn(`[V1] Failed to fetch leagues: ${(error as Error).message}`);
      return [];
    }
  }
